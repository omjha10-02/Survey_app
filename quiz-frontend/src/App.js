...
//     try {
//       const response = await axios.post(`${process.env.REACT_APP_BASE_URL}/api/signup`, { name, code });
...
//   useEffect(() => {
//     if (userId) {
//       axios.get(`${process.env.REACT_APP_BASE_URL}/api/quiz/questions`)
//         .then((response) => {
...
//       axios.post(`${process.env.REACT_APP_BASE_URL}/api/quiz/responses`, {
...
const App = () => {
  ...
  const [timeUp, setTimeUp] = useState(false); // Track if time is up
  const navigate = useNavigate();
    const baseUrl = process.env.REACT_APP_API_BASE_URL || 'https://survey-app-b003.onrender.com';
  ...
  const handleSubmitAnswer = () => {
    if (selectedOption) {
      ...
      axios.post(`${baseUrl}/api/quiz/responses`, {
        questionId: currentQuestion._id,
        selectedOption,
        userId,
      })
  ...
  useEffect(() => {
    if (timeUp) {
      toast('Time is up! Redirecting to results page...');
      navigate('/results'); // Redirect to results page when time is up
    }
  }, [timeUp, navigate]);
